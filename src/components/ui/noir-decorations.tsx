...
export function NoirDecorations({ className }: NoirDecorationsProps) {
  return (
    <div className={cn("fixed inset-0 pointer-events-none overflow-hidden z-0", className)}>
      ...
      <svg
        className="absolute -bottom-10 -left-10 w-80 h-80 opacity-[0.12]"
        viewBox="0 0 100 100"
      >
        <g fill="none" stroke="currentColor" strokeWidth="0.8" className="text-blue-500">
          {[...Array(15)].map((_, i) => (
            <ellipse
              key={i}
              cx="50"
      ...
      <div className="absolute top-1/4 left-12 -rotate-12 opacity-[0.06] text-[#2979FF] font-mono text-5xl font-bold tracking-widest">
        CLASSIFIED
      </div>

      {/* Second stamp - bottom right */}
      <div className="absolute bottom-1/4 right-8 rotate-6 opacity-[0.05] text-red-600 font-mono text-3xl font-bold tracking-widest">
        TOP SECRET
      </div>

      {/* Fingerprint - top right area */}
      <svg
        className="absolute top-40 right-48 w-48 h-48 opacity-[0.09]"
        viewBox="0 0 100 100"
      >
        <g fill="none" stroke="currentColor" strokeWidth="0.6" className="text-[#00E5FF]">
          {[...Array(12)].map((_, i) => (
            <ellipse
              key={i}
              cx="50"
      ...
      <svg
        className="absolute top-1/2 -right-20 w-64 h-64 opacity-[0.07]"
        viewBox="0 0 100 100"
      >
        <g fill="none" stroke="currentColor" strokeWidth="0.5" className="text-blue-500">
          {[...Array(18)].map((_, i) => (
            <ellipse
              key={i}
              cx="50"
      ...
      <div className="absolute bottom-20 left-1/2 -translate-x-1/2 opacity-[0.1]">
        <div className="w-12 h-12 bg-[#2979FF] rounded-sm flex items-center justify-center text-white font-bold text-2xl font-mono">
          1
        </div>
      </div>
    </div>
